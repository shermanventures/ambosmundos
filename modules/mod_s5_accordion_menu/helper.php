...
class modMainMenuHelper2
{
	function buildXML($params)
	{
		$menu = new JMenuTree2($params);
		$items = &JSite::getMenu();
	...
	function &getXML($type, $params, $decorator)
	{
		...
		if (!isset($xmls[$type])) {
			...
			$string = $cache->call(array('modMainMenuHelper2', 'buildXML'), $params);
			$xmls[$type] = $string;
		}
	...
	function render($params, $callback)
	{
		switch ( $params->get( 'menu_style', 'list' ) )
		{
			...
			default :
				...
				$xml = modMainMenuHelper2::getXML($params->get('menutype'), $params, $callback);
				if ($xml) {
 ...
 * @since		1.5
 */
class JMenuTree2 extends JTree
{
	/**
	...
	{
		$this->_params		=& $params;
		$this->_root		= new JMenuNode2(0, 'ROOT');
	...
	function addNode($params, $item)
	{
		...
		$node = new JMenuNode2($item->id, $item->name, $item->access, $data);
	...
	function _getLevelXML($depth)
	{
		...
		if ($depth <= 1) {
			$this->_buffer .= '<h3 class="s5_am_toggler" access="'.$this->_current->access.'" level="'.$depth.'" id="'.$this->_current->id.'"'.$rel.'>';} 
		else {
				$this->_buffer .= '<li class="s5_am_inner_li" access="'.$this->_current->access.'" level="'.$depth.'" id="'.$this->_current->id.'"'.$rel.'>';}
		...
		// Finish the item
		if ($depth <= 1) {
			$this->_buffer .= '</h3>';} else {
				$this->_buffer .= '</li>';} 
		
		if ($depth <= 1) {
		$this->_buffer .= '<div class="s5_accordion_menu_element" style="display: none; border:none; overflow: hidden; padding: 0px; margin: 0px"> ';}
		...
		while ($this->_current->hasChildren())
		{
			...
			if ($depth <= 1) {
				$this->_buffer .= '<ul class="s5_am_innermenu">';} else {
					
				$this->_buffer .= '<li><ul class="s5_am_innermenu">';}
		...
		if ($tmp->url != null)
		{
			...
			if ($tmp->home == 1) {
				...
				switch ($tmp->browserNav)
				{
					...
					case 0:
						// _top
						$data = '<span class="s5_accordion_menu_left"><a class="mainlevel" href="'.$tmp->url.'">'.$image.$tmp->name.'</a></span>';
						break;
					case 1:
						// _blank
						$data = '<span class="s5_accordion_menu_left"><a class="mainlevel" href="'.$tmp->url.'" target="_blank">'.$image.$tmp->name.'</a></span>';
						break;
					case 2:
						// window.open
