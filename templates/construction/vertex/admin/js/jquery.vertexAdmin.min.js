...
jQuery(document).ready(function() {
    var s5_menu_type = jQuery('#paramsxml_s5_menu_type');
    s5_menu_type.attr('name', 'xml_s5_menu_type');
    var s5_menu_type_id = s5_menu_type.attr('id');
    s5_menu_type_id = s5_menu_type_id.replace('paramsxml_', '');
    s5_menu_type.attr('id', s5_menu_type_id);
    var s5_hide_component_items = jQuery('#paramsxml_s5_hide_component_items');
	s5_hide_component_items.attr('name', 'xml_s5_hide_component_items');
    jQuery.ajax({
        type: 'GET',
        url: json_path,
        dataType: 'json',
    ...
    var style_form = jQuery('#element-box').find('form').css({width: '50%', 'float': 'left'}).find('.width-50').removeClass('width-50');
    var write_block = jQuery('#element-box').find('form').find('table.paramlist').parent().parent().parent().parent().parent().parent().find('.adminform:first');
    var write_block_class = write_block.find('font').attr('color');
    var write_block_text = write_block.html();
    write_block.remove();
    var old = jQuery('#element-box').find('form').find('table.paramlist').parent().parent().parent().parent();
    jQuery(old).remove();
    var write_block_new = jQuery('<div />').addClass(write_block_class).html(write_block_text);
    var new_admin = jQuery('<div />').attr('id', 'vertex_admin_wrap').css({width: '50%', 'float': 'left'});
    jQuery('#element-box').find('form').after(new_admin);
    ...
    var send_data = {state: 'start', style: template_name, style_name: template_name, vertex_xml: '/Vertex.xml', template_xml: '/templateDetails.xml', image_path: img_path};
        jQuery.ajax({
            type: 'POST',
            url: vertex_ajax_url + '/vadmin.php',
            data: send_data,
            success: function(data) {
                jQuery('#vertex_admin_wrap').empty().append(data);
                var form = jQuery('#vertex_admin_form');
                
                jQuery('#toolbar-apply').children().click(function(e){
                    jQuery.ajax({
                        type: 'POST',
                        url: vertex_ajax_url + '/saveOptions.php',
                        dataType: 'json',
                        data: {vertex: jQuery(form).serializeArray(), style: template_name, style_name: template_name},
                        success: function(data) {
                ...
                jQuery('#toolbar-save-copy').children().click(function(){
                    jQuery.ajax({
                        ...
                        data: {vertex: jQuery(form).serializeArray(), style: template_name, style_name: template_name},
                        success: function(data) {
							javascript:Joomla.submitbutton('style.save2copy');
						}
                    });
                    return false;
                });
				var addme = jQuery('#vertex_admin_wrap').find('.vItem').each(function(){
        ...
        jQuery(document).bind('vertex', function () {
            ...
            jQuery(function() {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: vertex_ajax_url + '/vertexUpdate.php',
                    dataType: 'json',
                    async: false,
                    success: function(data) {
                        update_link = data.update_link;
                        jQuery('#showVertexMsg').empty().css({height: 50, width: 310}).append(data.update_link);
                        jQuery('#showVertexMsg').unbind('click');
                    }
                });
                /*
                //jQuery('#showVertexMsg').click(function(){
                    var loader = jQuery('<img />').attr({src: vertex_ajax_url + '/df-images/loading.gif', height: 20, width: 220});
                    jQuery('#showVertexMsg').empty().append(loader);
                    jQuery.ajax({
                        type: 'POST',
                        url: vertex_ajax_url + '/vertexUpdateVertex.php',
                        dataType: 'json',
                        data: {version: vertex_version},
                        success: function(data) {
                            update_url = data.update_url;
                            jQuery('#showVertexMsg').empty().append(data.message);
                            jQuery('#showVertexMsg').unbind('click');
                            if(data.update == true) {
                                jQuery(document).trigger('vertex_update');
                            }
                        }
                    });
                //});
                
                jQuery(document).bind('vertex_update', function () {
                    var update_frame = jQuery('<iframe />').attr({src: vertex_ajax_url + '/vertexUpdate.php?do_update=true&update_url=' + update_url, height: 200, width: 220});
                    jQuery('#showVertexMsg').css({height: 200}).empty().append(update_frame);
                });
                jQuery(document).bind('vertex_updated', function () {
                    jQuery('#showVertexMsg').css({height: 20}).empty().append('Update Complete');
                });*/
                //jQuery('.vertex-admin-wrap .adminformlist li').each(function(){var a='/spacer/i';var b=jQuery(this).find('label');if(b.attr('id').match(/spacer/i)){var c=b.attr('id').match(/notice|alert|error|success|custom/i);var d=jQuery(this).find('label').html();jQuery(this).find('label').parent('div').remove();if(c){var e=jQuery('<img />').attr('src',vertexAdminImgPath+c+'.png').addClass('v-display-img-'+c).css({width:30,height:30,margin:'0 10px 0 5px'});var f=jQuery('<div />').addClass('v-display '+c).append(jQuery('<div />').addClass('vertex-title-text').append(d));jQuery(this).append(f.prepend(e))}else{}}});
            });
        });
});
function vertexUpdated(){
    jQuery(document).trigger('vertex_updated');
}
function vertexUpdatePercent(val){
    jQuery('#progressBar').empty().append('Percent Complete: ' + val);
}
/* Joomla Only - End */