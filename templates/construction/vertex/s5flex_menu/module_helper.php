...
class S5ModuleHelper
{
	...
	function &getModules($position)
	{
		...
		if(count($result) == 0) {
			if(JRequest::getBool('tp')) {
				$result[0] = S5ModuleHelper::getModule( 'mod_'.$position );
	...
	function &_load()
	{
		...
		//static $modules; - silviu removed the static $modules in order to remove the duplicate module title
