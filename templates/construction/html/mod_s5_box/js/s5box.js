var S5Box = new Class({
	...
	initialize: function(selector, options){
		...
		if(elements.length){			
			...
			if(!isInitialized){
				this.cbox.fx = new Fx.Style(this.cbox, 'opacity', {onComplete: function(){ if(that.ieVersion() > 0){ that.content.setStyle('filter', '');}}});
				this.overlay.fx = new Fx.Style(this.overlay, 'opacity', {duration: 300});
	...
	position: function (mWidth, mHeight, speed, loadedCallback) {
		...
		if(!this.cbox.fxs){
			var fx = new Fx.Styles(this.cbox, {
				duration: duration,
				wait: true
			});
		...
		if(duration == 0){
			...
			this.cbox.fxs.stop(true).setOptions({duration: duration}).start({
				height: this.wrap.mHeight,
				width: this.wrap.mWidth,
				top:posTop,
				left:posLeft
	...
	dimensions: function (object) {
		...
		function setPosition(s){
			...
			that.position(mWidth, mHeight, s, function(){
				...
				if (that.ieVersion() > 0){
					if(photo){
						new Fx.Style(that.loaded, 'opacity', {duration: 100, wait: false}).start(1);
					}
	...
	closefn: function(){
		...
		this.cbox.open = false;
		window.removeEvent('resize', this.events.windowResize);
		window.removeEvent('resize', this.events.windowResizeIE6);
		window.removeEvent('scroll', this.events.windowScrollIE6);
